...
async function main() {
  ...
  const manifest = {
    ...
    commands: [
      {
        ...
        command: `node overseas_movie_test/src/collect_boxofficemojo_us_release_schedule_full.mjs --config ${configRelative}`,
        status: "ready",
      },
      {
        step: "collect_raw_backbone",
        command: `node overseas_movie_test/src/collect_boxofficemojo_us_release_schedule_full.mjs --config ${configRelative} --execute`,
        status: "intentionally_blocked",
      },
      {
        step: "enrich_metadata",
        command: `node overseas_movie_test/src/build_complete_overseas_dataset.mjs --input-csv ${config.enrichment.inputCsv} --output-prefix ${config.enrichment.outputPrefix}`,
        status: "ready_after_collection",
      },
      {
        step: "filter_general_movies",
        command:
          `node overseas_movie_test/src/filter_overseas_general_movies.mjs ` +
      ...
      },
      {
        ...
        command:
          `node overseas_movie_test/src/build_overseas_analysis_summaries.mjs ` +
      ...
      },
      {
        ...
        command:
          `powershell -NoProfile -ExecutionPolicy Bypass -File overseas_movie_test/src/build_overseas_assignment1_visuals.ps1 ` +
    ...
    planned_outputs: {
      ...
      enriched_analysis_csv: `data/analysis_ready/${config.enrichment.outputPrefix}_analysis_ready.csv`,
    ...
    notes: [
      "This manifest prepares the full U.S. theatrical general-movie pipeline only.",
      "No collection has been started.",
      "The raw collector step remains intentionally blocked until we explicitly allow execution.",
    ],
  ...
  console.log(`Collection has not started. Raw collector step is still blocked by design.`);
}
